...
def d_teq_tp_strong(a, b, tp, l, lp, N, Np, d, cri, s, to): #function returning the phenotypic distance between the two species at equilibrium
    ...
    teq = to #set initial trait in species 1
    peq = to #set initial preference in species 1
    ...
    dX = np.array([1, 1])
    i = 0
    while np.sqrt(dX[0]**2 + dX[1]**2) > 0.000000000000001 and X[0]**2 + X[1]**2 < 10000000: #while equilibrium is not reached
        i+= 1
